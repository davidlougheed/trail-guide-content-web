...
const normalizeHTMLContent = (content: string | null | undefined): string => {
  if (!content) return "";
...
const contentItemField = (i: number) => (k: string) => `contents_${i}_${k}`;
...
const StationForm = React.memo((props: BaseParentObjectFormProps<Station>) => {
  ...
  const getContentKey = useCallback((k: string, i: number) => ({
  ...
  const transformFinalValues = useCallback((v) => ({
    ...
    contents: (v.contents.map(normalizeContents) ?? []).map((c, ci: number) => {
  ...
  const objectForm = useObjectForm<Station>({
            ...
            <Form.Item noStyle={true}>
              ...
              <Form.Item shouldUpdate={contentFieldChanged(field, "content_type")}
                ...
                {f => {
                  ...
                  const updateContentRef = (fk: string) => (el: ReactQuill) => {
